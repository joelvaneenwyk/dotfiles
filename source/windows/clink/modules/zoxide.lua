...
function __z(args)
    return ZOXIDE_EXE_PATH .. ' ' .. args
end

function __zoxide(args)
    local cmd = __z(args)
    print('##[cmd] ' .. cmd)
    return io.popen(cmd)
end
...
function __zoxide_query(options, keywords)
    options = table.concat(options, ' ')
    keywords = table.concat(keywords, ' ')

    local ok = false
    local result = nil
    local file = io.popen(__z('query ' .. options .. ' -- ' .. keywords))
...
function __zoxide_add(dir)
    os.execute(__z('add -- "' .. dir:gsub('^(.-)\\*$', '%1') .. '"'))
end
...
local function load_zoxide()
    local home = os.getenv("HOME") or os.getenv("USERPROFILE")
    local local_zoxide_executable = ""
    local loaded = false
    local values = {home .. "/.cargo/bin/zoxide.exe", home .. "/AppData/Local/Microsoft/WinGet/Links/zoxide.exe"}

    for key, value in pairs(values) do
        if os.isfile(local_zoxide_executable) then
            break
        else
            if local_zoxide_executable ~= "" then
                logger.debug('Zoxide not found: ' .. local_zoxide_executable)
            end
            local_zoxide_executable = path.normalise(value)
        end
    end

    if not os.isfile(local_zoxide_executable) then
        logger.warning('Zoxide not found: ' .. local_zoxide_executable)
    end

    local zoxide_version = nil
    local zoxide_path = nil
    if os.isfile(local_zoxide_executable) then
        zoxide_path = "\"" .. local_zoxide_executable .. "\""
        io.popen(zoxide_path .. " --version")
        if not os.geterrorlevel == 0 then
            logger.warning('Zoxide version test failed: \'' .. local_zoxide_executable .. '\'')
        else
            local process = io.popen(zoxide_path .. " --version")
            if process ~= nil then
                zoxide_version = all_trim(process:read("*a"))
                loaded = process:close()
            end
    ...
    if loaded then
        ZOXIDE_EXE_PATH = zoxide_path
        logger.info('Found Zoxide: ' .. local_zoxide_executable .. ' (' .. zoxide_version .. ')')
    else
        logger.error('Failed to find Zoxide: ' .. local_zoxide_executable)
    end

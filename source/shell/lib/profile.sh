...
initialize_interactive_profile() {
    ...
    *) ;;
    ...
    alias pgptest='. "$MYCELIO_ROOT/source/bin/pgptest.sh"'
    alias gpgtest='. "$MYCELIO_ROOT/source/bin/pgptest.sh"'
    alias cls='clear'
    ...
    alias refresh='git -C "$MYCELIO_ROOT" pull >/dev/null 2>&1 || true; . "$MYCELIO_ROOT/packages/shell/.profile"'
    ...
    if [ -z "${MYCELIO_PROFILE_INITIALIZED:-}" ]; then
        ...
        echo "▓├═════════════════════════════════"
        echo ""
        echo "[mycelio] Initialized '${MYCELIO_OS_NAME:-UNKNOWN}:${MYCELIO_OS_VARIANT:-UNKNOWN}' environment: '${MYCELIO_ROOT:-}'"
        echo "[mycelio] Parent Process: '$_parent'"
        echo ""
